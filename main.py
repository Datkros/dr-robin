...
class MainWindow(QMainWindow):
	...
	def creaModelo(self,todasMascotas,labels,mascota_id):
		model = QtGui.QStandardItemModel(parent=None)
	...
	def muestraInfo(self,mascota_id):
		def calculate_age(nacimiento):
			k = nacimiento.split('-')
			birthday = date(year=int(k[0]),month=int(k[1]),day=int(k[2]))
			today = date.today()
			if (today.year - birthday.year - ((today.month, today.day) < (birthday.month, birthday.day))) == 0:
				...
				return (str(today.year - birthday.year - ((today.month, today.day) < (birthday.month, birthday.day))) + " años").decode('utf-8')
