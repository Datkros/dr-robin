...
class SQL_Requests():
	...
	def muestraInfo(self,mascota_id):
		self.cur.execute(
			'''SELECT tipo_mascotas.nombre, mascotas.nombre, mascotas.raza, mascotas.color, mascotas.sexo, mascotas.nacimiento, pesos.peso
				FROM mascotas 
				JOIN tipo_mascotas, Pesos ON tipo_mascotas.id = mascotas.tipo_mascota_id AND mascotas.id = pesos.mascota_id
				WHERE mascotas.id = ?
	...
	def registraMascota(self, nombre, especie, sexo, nacimiento, raza, peso, color):
		...
		self.cur.execute('''SELECT id from Mascotas 
									WHERE  nombre = ? 
	...
	def obtienePeso(self,mascota_id):
		self.cur.execute('''SELECT peso, fecha_peso FROM Pesos WHERE mascota_id = ? ORDER BY fecha_peso ASC''', (mascota_id,))
		return self.cur.fetchall()
		
	def editaMascota(self, nombre, especie, sexo, nacimiento, raza, peso, color,mascota_id):
		self.cur.execute('''SELECT id FROM tipo_mascotas WHERE nombre = ? ''', (especie,))
		tipo_mascota_id = self.cur.fetchone()[0]
		self.cur.execute('''UPDATE Mascotas
						SET nombre = ?,
						raza = ? , 
						color = ?,
						sexo = ?,
